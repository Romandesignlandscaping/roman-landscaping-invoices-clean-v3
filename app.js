...
const STORAGE_KEY = 'roman-landscaping-clean-v3';
...
function defaultState() {
  ...
  return {
    ...
    settings: {
      ...
      promoImage: '/assets/promo-default-v3.png',
      headerLogo: '/assets/header-logo-v3.png',
      defaultDueDays: 14,
      compactView: false,
...
async function warmImages() {
  const paths = ['/assets/header-logo-v3.png', '/assets/promo-default-v3.png'];
    ...
    <div class="app-shell">
      <header class="topbar">
        <div class="brand-wrap">
          <div class="brand-side"><img src="/assets/icon-192.png" alt="Roman logo"></div>
  ...
  return `
    ...
    <section class="grid-2">
      ...
      <div class="section-card">
        ...
        <div class="preview-box"><img id="promoPreview" src="${state.settings.promoImage || '/assets/promo-default-v3.png'}" alt="Promo preview"></div>
...
function createInvoiceSnapshot(invoice) {
  ...
  return {
    ...
    promoImage: state.settings.promoImage || '/assets/promo-default-v3.png',
    headerLogo: state.settings.headerLogo || '/assets/header-logo-v3.png',
...
function saveInvoice({ download }) {
  ...
  saveState();
  if (download) generateInvoicePdf(invoice, { download: true });
  renderApp();
...
function previewCurrentInvoice() {
  ...
  generateInvoicePdf(invoice, { download: true });
}
...
async function generateInvoicePdf(invoice, { download = true, doc = null, addPage = false } = {}) {
  ...
  const headerLogoPath = snap.headerLogo || state.settings.headerLogo || '/assets/header-logo-v3.png';
  const headerLogo = await loadImageAsDataUrl(headerLogoPath);
  const promoPath = snap.promoImage || state.settings.promoImage || '/assets/promo-default-v3.png';
  ...
  [headerOwner, headerAddress, headerEmail].forEach((cell, index) => {
    ...
    localDoc.text(String(cell || ''), x + 8, y + 13);
    x += width;
  });
  y += rowHeight + 22;
  ...
  localDoc.text(`DATE: ${displayDate}`, margin + 130, y);
  const maxDescWidth = pageWidth - (margin + 275) - margin;
  const desc = localDoc.splitTextToSize(`DESCRIPTION: ${descriptionText}`, maxDescWidth)[0] || 'DESCRIPTION:';
  localDoc.text(desc, margin + 275, y);
  y += 10;
  ...
  const amountX = pageWidth - margin - 24;
  const textWidth = amountX - textX - 70;
  ...
  const blockX = pageWidth - margin - 190;
  localDoc.setFont('helvetica', 'normal');
  localDoc.setFontSize(10.5);
  localDoc.text(`Subtotal: ${money(subtotal)}`, blockX, y);
  y += 14;
  localDoc.text(`Tax: ${money(tax)}`, blockX, y);
  y += 14;
  localDoc.setFont('helvetica', 'bold');
  localDoc.text(`Total: ${money(total)}`, blockX, y);
  y += 18;
  ...
  let promoWidth = Math.min(contentWidth - 120, 460);
  ...
  if (promoHeight < 150) {
    ...
    localDoc.rect(0, 0, pageWidth, pageHeight, 'F');
    y = 40;
    promoWidth = Math.min(contentWidth - 120, 460);
